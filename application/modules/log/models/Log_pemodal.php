<?php
	...
	class Log_pemodal extends CI_Model {
	    ...
	    public function GetLogPemodal($param, $disable_page=false){
			$args["search"] = "";
            $return_data["data"] = "";
			...
			if(!empty($param["filter"]["id"])) {
				...
				$return_data["data"] = $this->db->get("v_log_pemodal_2021")->result();
			} else {
				$return_data["data"] = $this->db->get("v_log_pemodal_2021")->result();
	            // START SEARCH & FILTER 
				if(!empty($param["Search"])) {
	            ...
	            $return_data["paging"]["Total"] = $this->db->get("v_log_pemodal_2021")->num_rows();
		...
		public function HtmlLogPemodal($param){
		    $rHtml ="";
			$query = $this->GetLogPemodal($param);
			if(!empty($query["data"])) {
