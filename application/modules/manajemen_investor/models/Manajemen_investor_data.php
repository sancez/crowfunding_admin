<?php
	...
	class Manajemen_investor_data extends CI_Model {
		...
		public function HtmlManajemenInvestor($param){
			...
			if(!empty($query["data"])) {
				foreach ($query["data"] as $item) {
					...
					$btn_status_delete = "<a class='dropdown-item inactive-manajemen-investor' data-id='".$item->id."'>Inactive</a>";
					if($item->status_delete == 1){
						$status_delete = "<span class='badge badge-pill badge-danger'>Inactive</span>";
						$btn_status_delete = "<a class='dropdown-item active-manajemen-investor' data-id='".$item->id."'>Active</a>";
					}
                    $rHtml .= "<tr>
