<?php
	...
	class Manajemen_proyek_data extends CI_Model {
	    ...
	    public function GetManajemenProyek($param, $disable_page=false){
			$args["search"] = "";
            $return_data["data"] = "";
		...
		public function HtmlManajemenProyek($param){
		    $rHtml ="";
			$query = $this->GetManajemenProyek($param);
			...
			if(!empty($query["data"])) {
				foreach ($query["data"] as $item) {
                    ...
                    $rHtml .= "<tr>
								    <td><a href='".base_url()."/manajemen_proyek/detail.html?id=".$item->id."'><center style='max-height: 130px; overflow: hidden;'>".$foto."</center></a></td>
								    <td>
								    	<label class='bold'><a href='".base_url()."/manajemen_proyek/detail.html?id=".$item->id."'>".$item->nama."</a></label><br><br>
