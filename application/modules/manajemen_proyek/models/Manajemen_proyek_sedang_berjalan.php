<?php
	...
	class Manajemen_proyek_sedang_berjalan extends CI_Model {
	    ...
	    public function GetManajemenProyekSedangBerjalan($param, $disable_page=false){
			$args["search"] = "";
            $return_data["data"] = "";
		...
		public function HtmlManajemenProyekSedangBerjalan($param){
		    $rHtml ="";
			$query = $this->GetManajemenProyekSedangBerjalan($param);
			...
			if(!empty($query["data"])) {
				foreach ($query["data"] as $item) {
                    ...
                    if($item->status_active == 1){
                    	$status_active = "<a class='btn btn-outline-primary inactive-manajemen-proyek-sedang-berjalan mt-1' style='background-color: white !important; color: #3abaf4 !important; border: 1px solid #3abaf4 !important;' data-id='".$item->id."' data-nama='".$item->nama."'><i class='fa fa-eye'></i> Active</a>";
                    } else {
                    	$status_active = "<a class='btn btn-outline-primary active-manajemen-proyek-sedang-berjalan mt-1' style='background-color: white !important; color: #3abaf4 !important; border: 1px solid #3abaf4 !important;'  data-id='".$item->id."'  data-nama='".$item->nama."' ><i class='fa fa-eye-slash'></i> Inactive</a>";
                    }
                    ...
                    $rHtml .= "<tr>
								    ...
								    <td>
								    	...
								    	".$status_active."
								    	<a class='btn btn-outline-primary history-dana-manajemen-proyek-sedang-berjalan mt-1' style='background-color: white !important; color: #3abaf4 !important; border: 1px solid #3abaf4 !important;' data-id='".$item->id."'><i class='fa fa-file-invoice-dollar'></i> History Dana</a>
								    	<a class='btn btn-outline-success dividen-manajemen-proyek-sedang-berjalan mt-1' style='background-color: white !important; color: #28a745 !important; border: 1px solid #28a745 !important;' data-id='".$item->id."'><i class='fa fa-coins'></i> Dividen</a>
								    </td>
								    <td>
        ...
        public function UpdateManajemenProyekSedangBerjalan($id_update, $param) {
