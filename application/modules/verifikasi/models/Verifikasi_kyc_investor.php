<?php
	...
	class Verifikasi_kyc_investor extends CI_Model {
	    ...
	    public function GetVerifikasiKYCInvestor($param, $disable_page=false){
			$args["search"] = "";
            $return_data["data"] = "";
		...
		public function HtmlVerifikasiKYCInvestor($param){
		    $rHtml ="";
			$query = $this->GetVerifikasiKYCInvestor($param);
								    ...
								    <td class='text-center' style='padding-top:4px !important;'>
								        <div class='btn-group'>
								            ...
								            <div class='dropdown-menu'>
								                <a href='".base_url("/verifikasi/get_file_investor?id=").$item->id."' target='_blank' class='dropdown-item'  data-id='".$item->id."'>Download Data Upload</a>
								                <a href='".base_url("/verifikasi/get_personal_investor?id=".$item->id."&by=".$this->user->nama)."' target='_blank' class='dropdown-item' data-id='".$item->id."'>Download Data Personal</a>
								            </div>
								        </div>
								    </td>
        ...
        public function UpdateVerifikasiKYCInvestor($id_update, $param) {
        ...
        public function VerifikasiKYCInvestor($id_update, $param) {
        	...
        	if($param_update["status_verify"] == 1){
         	...
         	$this->db->where("id", $id_update);
		    $query = $this->db->update("tb_pemodal", $param_update);
		    if(!$query){
			   	$return_data["IsError"] = true;
				$return_data["ErrorMessage"] = $this->db->conn_id->error_list;
				return json_encode($return_data);
			    ...
			    $this->db->where("id_pemodal", $id_update);
			    $this->db->delete("tb_verifikasi_pemodal");
			    $param["id_pemodal"] = $id_update;
				$this->db->insert("tb_verifikasi_pemodal", $param);
			}
		   	return $query;
        }

        public function GetFileVerifikasiKYCInvestor($param){
