<?php
	...
	class Penerbit extends CI_Model {
	    ...
	    public function GetPenerbit($param, $disable_page=false){
			...
			if(isset($param["filter"]["id_penerbit"]) and $param["filter"]["id_penerbit"] != "") {
            	$this->db->where("id_penerbit", $param["filter"]["id_penerbit"]);
			}
		...
		/*public function HtmlPenerbit($param){
		    $rHtml ="";
			$query = $this->GetPenerbit($param);
			$array_id = array();
		...
		public function DropdownPenerbit($param) {
			$rHtml ="";
			$param["filter"]["status_verify"] = 1;
			$query = $this->GetPenerbit($param, true);
			if(!empty($query["data"])) {
				foreach ($query["data"] as $item) {
					 $rHtml .= '<option value="'.$item->id.'">'.$item->nama_perusahaan.'</option>';
				}
			} else {
