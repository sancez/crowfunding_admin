<?php
	...
	class Log_admin extends CI_Model {
	    ...
	    public function GetLogAdmin($param, $disable_page=false){
			$args["search"] = "";
            $return_data["data"] = "";
			...
			if(!empty($param["filter"]["id"])) {
				...
				$return_data["data"] = $this->db->get("v_log_admin_2021")->result();
			} else {
				$return_data["data"] = $this->db->get("v_log_admin_2021")->result();
	            // START SEARCH & FILTER 
				if(!empty($param["Search"])) {
	            ...
	            $return_data["paging"]["Total"] = $this->db->get("v_log_admin_2021")->num_rows();
		...
		public function HtmlLogAdmin($param){
		    $rHtml ="";
			$query = $this->GetLogAdmin($param);
			if(!empty($query["data"])) {
